...
If the <<features-presentTiming, pname:presentTiming>> feature is enabled,
and present timing collection has been enabled on pname:swapchain, calling
fname:vkWaitForPresentKHR guarantees that the present timing results
associated with pname:presentId are available to be collected by
